 ...
 let color_graph nodes adjacency colors =
   let rec color_graph_aux nodes colored succ fail =
     let rec color_node avail_colors =
       ...
       | (_, []) -> ask_usr show_coloring colored succ fail
       | ([], _) -> fail()
       | (col::rst_c, node::rst_n) -> if has_color adjacency node colored col
                                      then color_node rst_c
                                      else color_graph_aux rst_n ((node, col)::colored) succ (fun () -> color_node rst_c)
     in color_node colors
   in color_graph_aux nodes [] (fun () -> ()) (fun () -> Printf.printf "\nNo (more) colorings\n")
